...
class KalturaHelpers {
	...
	public static function push_json_ld_to_footer($entry){
		add_action("wp_footer",function() use ($entry){
			$json = array();
			$json['@context'] = "https://schema.org";
			$json['@type'] = "VideoObject";
			$json['name'] = $entry->name;
			$json['description'] = $entry->description;
			$json['isPartOf']['@id'] =  get_permalink();
			$json['thumbnailUrl'][0]=$entry->thumbnailUrl;
			$json['uploadDate'] = date("Y-m-dTH:i+00:00",$entry->updatedAt);
			$json['duration'] = KalturaHelpers::duration8601($entry->duration);
			$json['contentUrl'] = $entry->downloadUrl;
			$json['embedUrl'] = $entry->dataUrl;
	...
	public static function duration8601($second)
	{
	...
	public static function getCdnUrl($path = null) {
		$url = KalturaHelpers::getOption( 'kaltura_cdn_url' );
		$url = rtrim( $url, '/' );
		if ($path)
			$url .= $path;
	...
	public static function getEmbedOptions( $params ) {
		...
		$arrayKeys = array( 'width', 'height', 'uiconfid', 'align', 'wid', 'entryid', 'style', 'responsive', 'hoveringcontrols', 'isplaylist','mode' );
